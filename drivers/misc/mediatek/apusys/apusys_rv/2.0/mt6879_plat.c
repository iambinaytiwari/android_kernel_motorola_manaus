...
static int mt6879_irq_affin_init(struct mtk_apu *apu)
{
	int i;

	/* init perf_cpus mask 0xc0, CPU6~7 */
	cpumask_clear(&perf_cpus);
	cpumask_set_cpu(6, &perf_cpus);
	cpumask_set_cpu(7, &perf_cpus);

	/* init normal_cpus mask 0x3f, CPU0~CPU5 */
...
static int mt6879_irq_affin_set(struct mtk_apu *apu)
{
...
static int mt6879_irq_affin_unset(struct mtk_apu *apu)
{
	irq_set_affinity_hint(apu->mbox0_irq_number, &normal_cpus);
...
const struct mtk_apu_platdata mt6879_platdata = {
	...
	.ops		= {
		...
		.irq_affin_init = mt6879_irq_affin_init,
		.irq_affin_set = mt6879_irq_affin_set,
		.irq_affin_unset = mt6879_irq_affin_unset,
	},
};
